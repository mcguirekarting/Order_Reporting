...
from utils.mongo_report_config import get_active_report_ids, get_report_config
...
def get_active_reports(**kwargs):
    """
    Get the list of active report configurations from MongoDB and decide which branches to execute
    """
    try:
        active_report_ids = get_active_report_ids()
        if not active_report_ids:
            logger.warning("No active reports found in MongoDB")
            return ['no_active_reports']
        
        logger.info(f"Found {len(active_report_ids)} active reports from MongoDB: {active_report_ids}")
        
        # Return task IDs corresponding to each report
        return [f"process_report_{report_id}" for report_id in active_report_ids]
    
    except Exception as e:
        logger.error(f"Error retrieving active reports from MongoDB: {str(e)}")
        return ['no_active_reports']

# Function to query the API for a specific report
def query_report_data(report_id, **kwargs):
    """
    Query the API based on report configuration from MongoDB
    """
    # Get execution date from context
    execution_date = kwargs['execution_date']
    
    # Load report configuration from MongoDB
    try:
        report_config = get_report_config(report_id)
        if not report_config:
            raise ValueError(f"Report configuration not found in MongoDB for report_id: {report_id}")
    except Exception as e:
        logger.error(f"Error loading configuration for report {report_id} from MongoDB: {str(e)}")
        raise
...
def prepare_email(report_id, **kwargs):
    ...
    # Load report configuration from MongoDB to get email settings
    try:
        report_config = get_report_config(report_id)
        if not report_config:
            raise ValueError(f"Report configuration not found in MongoDB for report_id: {report_id}")
    ...
    # For Jinja templating in EmailOperator, we need both list and comma-separated string
    recipients_list = recipients
    recipients_str = ",".join(recipients)
    
    logger.info(f"[{report_id}] Sending report to {len(recipients_list)} recipients: {recipients_str}")
    
    # Return the email parameters
    # recipients_str is used by EmailOperator's 'to' field (which accepts comma-separated string or list)
    return {
        "file_path": pdf_file,
...
# Get the active report IDs at DAG definition time from MongoDB
# This is only used for setting up the task structure
# The actual active reports will be determined at runtime
try:
    active_report_ids = get_active_report_ids()
except:
    logger.warning("Failed to fetch active report IDs from MongoDB at DAG definition time")
    active_report_ids = []
...
for report_id in active_report_ids:
    with TaskGroup(group_id=f"process_report_{report_id}", dag=dag) as report_group:
        ...
        # render_template_as_native_obj=True ensures Jinja templates return native Python types (lists, dicts)
        email_task = EmailOperator(
            task_id=f"send_email_{report_id}",
            to="{{ ti.xcom_pull(task_ids='prepare_email_" + report_id + "')['recipients_list'] }}",
